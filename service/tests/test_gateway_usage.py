...
def attempt(tracker, event_kind="attempt_finished", **overrides):
    fields = dict(event_kind=event_kind, operation_id="op-1", attempt_id="attempt-1",
                  actor="alice", job_id="job-1", route="gateway", incident_id=None,
                  model_requested="Exact/Alias", outcome="success")
...
def test_additive_migration_preserves_legacy_rows_and_reopens_idempotently(tmp_path):
    ...
    with sqlite3.connect(path) as conn:
        ...
        conn.execute("INSERT INTO gemini_usage_log (timestamp, model, call_type, total_tokens) "
                     "VALUES (?, 'legacy', 'classify_batch', 7)", (utc_now_iso(),))
    ...
    for _ in range(2):
        ...
        with sqlite3.connect(path) as conn:
            ...
            assert {"event_kind", "operation_id", "attempt_id", "actor", "route",
                    "response_id", "incident_id", "model_requested", "model_actual",
                    "outcome", "error_category", "usage_known"} <= columns
...
def test_attempt_events_are_idempotent_and_summaries_count_finished_only(tmp_path):
    ...
    with UsageTracker(path) as tracker:
        ...
        for _ in range(2):
            ...
            attempt(tracker, usage=usage, estimated_cost_usd=0.01, response_id="response-1",
    ...
    with sqlite3.connect(path) as conn:
        ...
        assert conn.execute("SELECT response_id, model_actual, actor FROM gemini_usage_log "
                            "WHERE event_kind='attempt_finished'").fetchone() == (
                                "response-1", "actual-model", "alice")
...
def test_failed_usage_is_retained_and_unknown_is_not_a_success(tmp_path):
    with UsageTracker(tmp_path / "usage.db") as tracker:
        attempt(tracker, outcome="error", error_category="response_parse", response_id="response-1",
...
def test_usage_whitelist_retains_provider_counters_not_raw_payload(tmp_path):
    with UsageTracker(tmp_path / "usage.db") as tracker:
        attempt(tracker, usage={"prompt_tokens": 5, "completion_tokens": 2, "total_tokens": 10,
                                "thinking_tokens": 3, "cached_tokens": 1,
                                "prompt": "sensitive-content", "api_key": "sensitive-key"})
        usage = tracker.query_attempts()[0]["usage"]
        assert usage == {"prompt_tokens": 5, "completion_tokens": 2, "total_tokens": 10,
                         "thinking_tokens": 3, "cached_tokens": 1}
        assert "sensitive" not in " ".join(tracker._conn.iterdump())


@pytest.mark.parametrize("overrides", [
    {"event_kind": "invalid"}, {"attempt_id": ""}, {"operation_id": ""}, {"actor": ""},
    {"error_category": "raw error body\nsecret"},
    {"usage": {"prompt_tokens": -1}}, {"usage": {"total_tokens": 1.5}},
    {"usage": {"total_tokens": True}}, {"usage": {"total_tokens": "secret"}},
    {"estimated_cost_usd": -1}, {"estimated_cost_usd": float("nan")},
...
def test_duplicate_events_across_connections_use_database_uniqueness(tmp_path):
    ...
    def write_event(_):
        with UsageTracker(path) as tracker:
            attempt(tracker, usage={"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0},
