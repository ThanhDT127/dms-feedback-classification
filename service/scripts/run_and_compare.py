...
def run_all():
    ...
    runtime_settings = settings.model_copy(
        update={
            "keyword_dir_override": active_keyword
            if active_keyword.exists()
            else settings.keyword_dir,
    ...
    runner = PipelineRunner(gemini=gemini, rag=rag, metrics=metrics, settings=runtime_settings)
...
def compare_all():
    ...
    for fname in TEST_FILES:
        ...
        report_lines.append(
            f"- **Rows with label differences**: {total_label_mismatches} ({total_label_mismatches / n_rows * 100:.0f}%)"
        )
        report_lines.append(
            f"- **Sentiment match**: {sentiment_match}/{n_rows} ({sentiment_match / n_rows * 100:.0f}%)\n"
        )
        ...
        if detail_rows:
            ...
            report_lines.append(
                "| # | Text | Old Labels | New Labels | Old Sent | New Sent | Diff |"
            )
            report_lines.append("|---|---|---|---|---|---|---|")
