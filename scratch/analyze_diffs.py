...
output_lines = []
output_lines.append(
    f"Total differences found in DMS-13102025: {len(diff_rows)} / {n_rows} rows."
)
...
for idx, d in enumerate(diff_rows):
    ...
    output_lines.append(
        f"  - OLD: Labels={d['old_labels'] or ['Tin trung lập']} | Sentiment={d['old_sentiment']}"
    )
    output_lines.append(
        f"  - NEW (V3): Labels={d['new_labels'] or ['Tin trung lập']} | Sentiment={d['new_sentiment']}"
    )
