...
def main():
    ...
    for out_file in output_files:
        ...
        try:
            ...
            for idx, row in df_out.iterrows():
                ...
                if val and val != "nan" and len(val) > 15 and not val.startswith("col_") and val != "Nội dung vấn đề":
            ...
            records.append({
                ...
                "old_sentiment": str(old_sentiment) if pd.notna(old_sentiment) else ""
            })
    ...
    # Compile comparison
    comparison_lines = []
    comparison_lines.append("# Báo cáo Kiểm thử So sánh Prompt Cũ vs Prompt Mới (20 Ca Thực tế)")
    ...
    for idx, (rec, res) in enumerate(zip(records, new_results)):
        ...
        new_lbl_str = ", ".join(res["final_minors"]) if res["final_minors"] else "Không gán"
        ...
        if set(rec["old_labels"]) == set(res["final_minors"]):
            ...
            elif any(x in text_lower for x in ["chậm", "giao hàng", "thiếu"]) and "website" in rec["old_labels"] and "website" not in res["final_minors"]:
        ...
        comparison_lines.append(f"| {idx+1} | {rec['file']} | {rec['text']} | {old_lbl_str} | {new_lbl_str} | {comment} |")
