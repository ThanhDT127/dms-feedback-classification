...
report.append(
    f"Quy mô dữ liệu đối chiếu: {total_aligned} dòng phản hồi gộp từ 5 file Excel."
)
...
for r in v4_records:
    ...
    old_ok = (
        set(r["old_labels"]) == set(r["gt_labels"])
        and r["old_sentiment"] == r["gt_sentiment"]
    )
    v3_ok = (
        set(r["v3_labels"]) == set(r["gt_labels"])
        and r["v3_sentiment"] == r["gt_sentiment"]
    )
    v4_ok = (
        set(r["v4_labels"]) == set(r["gt_labels"])
        and r["v4_sentiment"] == r["gt_sentiment"]
    )
...
report.append("|---|:---:|:---:|:---:|")
report.append(
    f"| **Bản cũ (Old Production)** | {np.mean(old_exact) * 100:.2f}% | {np.mean(old_lbl_exact) * 100:.2f}% | {np.mean(old_sent_exact) * 100:.2f}% |"
)
report.append(
    f"| **Bản mới V3 (V3)** | {np.mean(v3_exact) * 100:.2f}% | {np.mean(v3_lbl_exact) * 100:.2f}% | {np.mean(v3_sent_exact) * 100:.2f}% |"
)
report.append(
    f"| **Bản hiện tại V4 (V4)** | {np.mean(v4_exact) * 100:.2f}% | {np.mean(v4_lbl_exact) * 100:.2f}% | {np.mean(v4_sent_exact) * 100:.2f}% |"
)
...
for f in TEST_FILES:
    ...
    report.append(
        f"| {f} | {tot} | {p_old:.1f}% | {p_v3:.1f}% | {p_v4:.1f}% | **+{growth_v3:.1f}%** | **+{growth_old:.1f}%** |"
    )
...
report.append("|---|:---:|:---:|:---:|:---:|")
report.append(
    f"| **Macro Accuracy** | {macro_old[0] * 100:.2f}% | {macro_v3[0] * 100:.2f}% | {macro_v4[0] * 100:.2f}% | **+{((macro_v4[0] - macro_v3[0]) * 100):.2f}%** |"
)
report.append(
    f"| **Macro Precision** | {macro_old[1] * 100:.2f}% | {macro_v3[1] * 100:.2f}% | {macro_v4[1] * 100:.2f}% | **+{((macro_v4[1] - macro_v3[1]) * 100):.2f}%** |"
)
report.append(
    f"| **Macro Recall** | {macro_old[2] * 100:.2f}% | {macro_v3[2] * 100:.2f}% | {macro_v4[2] * 100:.2f}% | **+{((macro_v4[2] - macro_v3[2]) * 100):.2f}%** |"
)
report.append(
    f"| **Macro F1-Score** | {macro_old[3] * 100:.2f}% | {macro_v3[3] * 100:.2f}% | {macro_v4[3] * 100:.2f}% | **+{((macro_v4[3] - macro_v3[3]) * 100):.2f}%** |"
)
...
for lbl in MINOR_ORDER:
    ...
    report.append(
        f"| {lbl} | {supp} | {f_old:.1f}% | {f_v3:.1f}% | {f_v4:.1f}% | {status} |"
    )
...
for r in v4_records:
    ...
    if any(x in t_low for x in ["sino", "sopoka", "mpe"]) and "rạng đông" in t_low:
        if (
            len(r["v4_labels"]) > 1
            and "Hãng" in r["v4_labels"]
            and any(
                lbl in ["Báo lỗi", "Y/c cải tiến", "Báo CL tốt"]
                for lbl in r["v4_labels"]
            )
        ):
            comp_ex.append(r)
...
for idx, r in enumerate(comp_ex[:3]):
    report.append(f'\n* **Ví dụ {idx + 1}:** "*{r["text"]}*"')
    report.append(
        f"  - **Bản cũ (Old):** `{r['old_labels']}` | Sent: `{r['old_sentiment']}`"
    )
    report.append(
        f"  - **Bản mới V3:** `{r['v3_labels']}` | Sent: `{r['v3_sentiment']}`"
    )
    report.append(
        f"  - **Bản hiện tại V4:** `{r['v4_labels']}` | Sent: `{r['v4_sentiment']}`"
    )
    report.append(
        f"  - **Ground Truth:** `{r['gt_labels']}` | Sent: `{r['gt_sentiment']}`"
    )
...
for r in v4_records:
    if (
        "cải tiến" in r["gt_labels"]
        and "Báo lỗi" not in r["gt_labels"]
        and "Báo lỗi" in r["v3_labels"]
        and "Báo lỗi" not in r["v4_labels"]
    ):
        err_ex.append(r)
    if (
        "Báo lỗi" in r["gt_labels"]
        and "cải tiến" not in r["gt_labels"]
        and "Báo lỗi" not in r["old_labels"]
        and "Báo lỗi" in r["v4_labels"]
    ):
        err_ex.append(r)
...
for r in v4_records:
    if (
        "tin thưởng" in r["text"].lower()
        or "thưởng" in r["text"].lower()
        and "Trả thưởng" in r["v3_labels"]
        and "Trả thưởng" not in r["v4_labels"]
    ):
        spell_ex.append(r)
