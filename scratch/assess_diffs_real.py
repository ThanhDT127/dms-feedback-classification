...
def parse_diff_file(file_path):
    ...
    first_block_data = (
        first_block[first_row_idx:] if first_row_idx != -1 else first_block
    )
...
def evaluate_record(r):
    ...
    if has_comp:
        ...
        if "Hãng" in old_lbl and len(old_lbl) == 1 and len(v3_lbl) > 1:
            return (
                "V3_Better",
                "V3 bóc tách chi tiết hơn đối thủ (thêm CTKM/TT SP/Hoạt động)",
            )
        if any(
            lbl in old_lbl for lbl in ["Bảng biển", "Trả thưởng", "Y/c cải tiến"]
        ) and not any(lbl in old_lbl for lbl in ["Hãng"]):
    ...
    if has_disp:
        if (
            "Bảng biển" not in old_lbl
            and "Kệ bóng, thử đèn,…" not in old_lbl
            and ("Bảng biển" in v3_lbl or "Kệ bóng, thử đèn,…" in v3_lbl)
        ):
    ...
    if has_prop:
        if ("Đề xuất SPM" in v3_lbl or "Y/c cải tiến" in v3_lbl) and not (
            "Đề xuất SPM" in old_lbl or "Y/c cải tiến" in old_lbl
        ):
    ...
    if old_sent != v3_sent:
        ...
        if "Báo lỗi" in v3_lbl and v3_sent == "Tiêu cực" and old_sent != "Tiêu cực":
            return (
                "V3_Better",
                "V3 sửa Sentiment sang Tiêu cực rất chuẩn xác cho lỗi sản phẩm",
            )
        if (
            ("Đề xuất SPM" in v3_lbl or "Y/c cải tiến" in v3_lbl)
            and v3_sent == "—"
            and old_sent == "Tiêu cực"
        ):
        ...
        if "Hàng hoá" in v3_lbl and v3_sent == "Tiêu cực" and old_sent == "—":
            return (
                "V3_Better",
                "V3 sửa Sentiment sang Tiêu cực cho phản ánh hàng bán chậm/tồn kho",
            )
...
res1 = run_analysis(
    r"D:\Works\DMS\scratch\diff_DMS-1810-1910.txt", "DMS-1810-1910.xlsx"
)
res2 = run_analysis(
    r"D:\Works\DMS\scratch\diff_DMS-2010-2210.txt", "DMS-2010-2210.xlsx"
)
