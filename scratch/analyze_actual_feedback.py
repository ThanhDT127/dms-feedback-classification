...
def analyze_data():
    ...
    label_cols = [
        ...
        "Bảng giá, Catalogue", "Bảng biển", "Kệ bóng, thử đèn,…", 
        "Khác", "Tốt/ ko tốt", "Trả thưởng", "Đề xuất", "Bảo hành", 
        "HTPP", "Hàng hoá", "Hàng giả", "Website", "Hãng", 
        "Hoạt động", "CTKM, giá, cơ chế", "TT SP", "Tin trung lập"
    ]
    ...
    for f in output_files:
        try:
            ...
            for idx, col in enumerate(df_headers.columns):
                if "nội dung vấn đề" in str(col).lower() or ("nội dung" in str(col).lower() and "mã" not in str(col).lower()):
                    text_idx = idx
                    break
            ...
            if text_idx is not None and text_idx < len(df.columns):
                ...
                for col in df.columns:
                    if col not in label_cols and col not in ["Sentiment", "LLM_Extracted", "BM25_Score"]:
            ...
            brand_idx = None
            for idx, col in enumerate(df_headers.columns):
                if "thương hiệu" in str(col).lower() or "brand" in str(col).lower() or "hãng" in str(col).lower():
                    brand_idx = idx
                    break
            brand_col = df.columns[brand_idx] if brand_idx is not None and brand_idx < len(df.columns) else None
            
            for idx, row in df.iterrows():
                text = row[comment_col]
                if pd.isna(text) or str(text).strip() == "" or str(text).strip().lower() == "nội dung vấn đề":
                    continue
                ...
                for label in label_cols:
                    val = row[label]
                    if pd.notna(val) and str(val).strip().lower() in ("x", "1", "true", "yes", "có"):
                ...
                feedback_corpus.append({
                    ...
                    "sentiment": str(sentiment).strip() if pd.notna(sentiment) else ""
                })
    ...
    abbrev_counts = {
        "bh": 0, "km": 0, "sp": 0, "đl": 0, "npp": 0, "c1": 0, "c2": 0, "bgn": 0, "at": 0, "tin thưởng": 0, "đèn": 0
    }
    ...
    for lbl, examples in label_examples.items():
        ...
        for ex in examples:
            ...
            print(f"    Labels: {ex['labels']}, Sentiment: {ex['sentiment']!r}, Brand: {ex['brand']!r}")
