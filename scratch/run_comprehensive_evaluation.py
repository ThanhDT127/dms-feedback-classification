...
def check_row_truth(r):
    ...
    competitors = ["mpe", "sopoka", "sino", "duhal", "tran phu", "trần phú", "g8", "nival", "ominsu", "omisu",
                   "nanoco", "okas", "hc lighting", "philips", "havaco", "vinasun", "elink", 
                   "panasonic", "pana", "lumi", "sunhouse", "việt nhật", "tý liên", "ty liên", "kaisu", "maxben", "du hal", "tiền phong"]
    ...
    is_ref = False
    if has_comp:
        ref_patterns = ["giống như", "giống hãng", "như hãng", "như đối thủ", "giống sofoka", "giống sopoka", "giống sino", "giống điện quang", "như điện quang", "giống hoa sen"]
    ...
    if has_comp and not is_ref:
        ...
        if any(k in t_lower for k in ["ck", "chiết khấu", "giá", "tặng", "khuyến mại", "km", "du lịch", "gói", "đạt vé", "thưởng", "hội nghị", "rẻ hơn", "giá buôn", "bán thấp hơn"]):
            gt_labels.append("CTKM, giá, cơ chế")
            
        # Hoạt động of competitor
        if any(k in t_lower for k in ["kệ", "trưng bày", "biển hiệu", "biển mới", "bảng", "hội nghị", "triển khai", "thay 100% biển"]):
            gt_labels.append("Hoạt động")
            
        # TT SP of competitor
        if any(k in t_lower for k in ["mẫu", "màu", "thanh đồng", "chân cắm", "phi 5", "lá đồng", "3 pha", "tương đương", "cốc", "ấm siêu tốc", "20a", "30a", "vợt bắt muỗi", "đèn bắt muỗi", "nhẹ hơn", "dòng 300w", "bọc trong màng co", "cầm lỏng lẻo"]):
            gt_labels.append("TT SP")
            
        gt_sent = "—"
        if any(k in t_lower for k in ["mẫu đẹp", "chất lượng tốt", "được bọc trong", "dễ vận chuyển", "gọn", "rẻ"]):
            gt_sent = "Tích cực"
    ...
    err_keywords = ["lỗi", "không sáng", "hư", "cháy", "không vào điện", "hỏng", "thụn đui", 
                    ...
                    "chập", "kém", "nhanh hỏng", "không tự ngắt", "sạc không đầy", "nhanh nguội", 
                    "gãy pass", "muội", "vô nước", "chết led", "thoát nhiệt", "yếu", "mùi tanh", "bị đơ", "khó ấn", "không nhạy", "lỏng lẻo"]
    if any(k in t_lower for k in err_keywords) or "giữ nhiệt kém" in t_lower:
        gt_labels.append("Báo lỗi")
        
    # 2. Y/c cải tiến
    imp_keywords = ["cải tiến", "cần làm", "nên làm", "làm mỏng", "nâng công suất", "in chữ nổi", 
                    "thêm nắp che", "làm dày", "thêm đèn", "dây dài hơn", "in thêm", "đóng thành dây", 
                    "thay đổi", "cần lỗ quét", "làm lớp nilon", "điều chỉnh", "nắp che an toàn", "in luôn số a"]
    if any(k in t_lower for k in imp_keywords):
        gt_labels.append("Y/c cải tiến")
        
    # 3. Đề xuất SPM
    spm_keywords = ["ra thêm", "sản xuất thêm", "sản xuất lại", "có thêm loại", "thêm size", "mã mới", "thêm loại", "ra dòng mới", "thêm dòng", "làm thêm bóng", "làm thêm tủ nhựa"]
    if any(k in t_lower for k in spm_keywords):
        gt_labels.append("Đề xuất SPM")
        
    # 4. Bảng giá, Catalogue
    if any(k in t_lower for k in ["catalogue", "cateloge", "câtloge", "báo giá", "bảng giá"]):
        gt_labels.append("Bảng giá, Catalogue")
        
    # 5. Bảng biển
    if any(k in t_lower for k in ["biển hiệu", "biển quảng cáo", "bảng quảng cáo", "làm biển", "cấp biển", "bảng hiệu", "làm bảng biển"]):
        gt_labels.append("Bảng biển")
        
    # 6. Kệ bóng, thử đèn,…
    if any(k in t_lower for k in ["kệ", "trưng bày", "demo", "bộ test", "bảng test", "kệ bày hàng", "tủ bày", "dụng cụ thử bóng", "kệ bóng"]):
        gt_labels.append("Kệ bóng, thử đèn,…")
        
    # 7. Bảo hành
    if any(k in t_lower for k in ["bảo hành", "đổi trả", "đổi mới", "ứng vật tư", "giải quyết bảo hành"]):
        gt_labels.append("Bảo hành")
        
    # 8. Hàng hoá
    if any(k in t_lower for k in ["bán chậm", "tồn kho", "giao hàng chậm", "thiếu hàng", "nhập chậm", "khó bán", "hàng chậm", "giao chậm", "ra chậm quá", "chưa nhập thêm"]):
        gt_labels.append("Hàng hoá")
        
    # 9. HTPP
    if any(k in t_lower for k in ["npp", "nhà phân phối", "tràn vùng", "phá giá", "đại lý giao", "hoàng quý", "quang phú", "hưng chiến", "tùng gia phát", "ngọc huy", "ngọc hiếu", "lê minh phát", "buôn hưng yên", "buôn thái bình", "nhập hàng từ"]):
        gt_labels.append("HTPP")
        
    # 10. Trả thưởng
    if any(k in t_lower for k in ["thưởng", "quay số", "trúng thưởng", "chương trình quay", "trả thưởng", "gói quay", "c2td", "nợ lâu"]):
    ...
    # 12. Tốt/ ko tốt
    if any(k in t_lower for k in ["giá", "cơ chế", "chiết khấu", "đắt", "rẻ", "hợp lý", "khó bán"]):
        if not any(lbl in gt_labels for lbl in ["Bảng giá, Catalogue", "Trả thưởng", "HTPP"]):
            gt_labels.append("Tốt/ ko tốt")
            
    # 13. Báo CL tốt
    if any(k in t_lower for k in ["chất lượng tốt", "ánh sáng ổn định", "ít khi bị lỗi", "chất lượng", "hiệu suất cao"]):
...
def evaluate_quality(records):
    ...
    for r in records:
        ...
        old_jaccard = len(old_lbl_set.intersection(gt_lbl_set)) / len(old_lbl_set.union(gt_lbl_set)) if len(old_lbl_set.union(gt_lbl_set)) > 0 else 1.0
        v3_jaccard = len(v3_lbl_set.intersection(gt_lbl_set)) / len(v3_lbl_set.union(gt_lbl_set)) if len(v3_lbl_set.union(gt_lbl_set)) > 0 else 1.0
...
def print_final_accurate_report(eval_list, name):
    ...
    v3_strictly_better = sum(1 for x in eval_list if x['better'] == "V3" and not x['old_correct'])
    old_strictly_better = sum(1 for x in eval_list if x['better'] == "Old" and not x['v3_correct'])
    both_correct = sum(1 for x in eval_list if x['old_correct'] and x['v3_correct'])
    both_incorrect_equal = sum(1 for x in eval_list if not x['old_correct'] and not x['v3_correct'] and x['better'] == "Both")
    
    print(f"\n=================== REPORT FOR {name} ===================")
    print(f"Total evaluated records: {total}")
    print(f"Old 100% correct accuracy: {old_correct_count} ({old_correct_count/total*100:.2f}%)")
    print(f"V3 100% correct accuracy: {v3_correct_count} ({v3_correct_count/total*100:.2f}%)")
    print(f"V3 strictly better (and Old incorrect): {v3_strictly_better} rows ({v3_strictly_better/total*100:.2f}%)")
    print(f"Old strictly better (and V3 incorrect): {old_strictly_better} rows ({old_strictly_better/total*100:.2f}%)")
    print(f"Both 100% correct: {both_correct} rows ({both_correct/total*100:.2f}%)")
    print(f"Both incorrect and equally bad: {both_incorrect_equal} rows ({both_incorrect_equal/total*100:.2f}%)")
