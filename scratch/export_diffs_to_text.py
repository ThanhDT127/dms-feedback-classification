...
for fname in TEST_FILES:
    ...
    output_lines.append(f"Total differences: {len(diff_rows)} / {n_rows} rows\n")
    for idx, d in enumerate(diff_rows):
        output_lines.append(f"Row: {d['row_index']}")
        output_lines.append(f"Text: {d['text']}")
        output_lines.append(
            f"  Old: {d['old_labels']} | Sentiment: {d['old_sentiment']}"
        )
        output_lines.append(
            f"  V3:  {d['new_labels']} | Sentiment: {d['new_sentiment']}"
        )
        output_lines.append("-" * 50)

    out_file = script_dir / "scratch" / f"diff_{fname.replace('.xlsx', '.txt')}"
    out_file.write_text("\n".join(output_lines), encoding="utf-8")
    print(f"Exported {len(diff_rows)} diffs of {fname} to {out_file}")
