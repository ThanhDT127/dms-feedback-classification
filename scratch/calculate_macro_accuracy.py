...
for label in MINOR_ORDER:
    ...
    y_old = (
        ...
        .astype(str)
        .str.strip()
        .apply(lambda x: 1 if x not in ("", "nan", "None") else 0)
        .values
    )
    y_new = (
        df_all_new[label]
...
valid_labels = [
    lbl
    for lbl, res in label_results.items()
    if res["Support"] > 0 or (res["TP"] + res["FP"] > 0)
]
...
report_lines.append(
    f"- **Macro-average Accuracy (Độ chính xác trung bình từng nhãn của Bản cũ):** {macro_accuracy * 100:.2f}%"
)
report_lines.append(
    f"- **Macro-average F1-Score (Điểm F1 trung bình của Bản cũ):** {macro_f1 * 100:.2f}%"
)
report_lines.append(
    f"- **Macro-average Precision (Độ xác thực trung bình của Bản cũ):** {macro_precision * 100:.2f}%"
)
report_lines.append(
    f"- **Macro-average Recall (Độ phủ trung bình của Bản cũ):** {macro_recall * 100:.2f}%"
)
...
for label in MINOR_ORDER:
    ...
    if supp == 0 and res["FP"] == 0:
        ...
        quality = (
            "Sai hoàn toàn / Bỏ sót ❌❌" if supp > 0 else "Hoàn hảo (Không có mẫu)"
        )

    report_lines.append(
        f"| {label} | {acc:.1f}% | {prec:.1f}% | {rec:.1f}% | {f1:.1f}% | {supp} | {quality} |"
    )
